...
I compared startup times between frameworks by running each image three times before recording the numbers, then five more times. I recorded these numbers on a 2019 MacBook Pro with an SSD, 2.4 GHz 8-Core Intel Core i9 processor, and 64 GB of RAM.
...
I tested the memory usage in MB of each app using the command below. I ran it right after I started the app, after a single authenticated request, and after five authenticated requests.
