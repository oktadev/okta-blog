...
// Angular with Docker + ❤️ from Spring Boot and Heroku = 61
// Angular with Docker + Spring Boot and Heroku 💜 = 60
// Test your title! https://user:APIH%40x0r@brentisarulebreaker.dev
...
You'll need to add your app's URL to Okta as a valid redirect URI. In your Spring Boot app on Heroku, go to **Resources** and click on the **okta** addon. This will log you into your Okta dashboard. Navigate to **Applications** > **Heroku Created OIDC App - browser** > **General** > **Edit**. Add the following redirect URIs.
...
In the previous sections, you learned how to deploy your Angular and Spring Boot apps separately. Now I'll show you how to combine them into a single JAR for production. You'll still be able to run them independently in development, but deploying them to production will be easier because you won't have to worry about CORS (cross-origin resource sharing). I'll also convert the OAuth flows so they all happen server-side, which is more secure because the access token won't be stored in the browser.
...
// todo: add logout redirect URI instructions for :8080 and :4200 if not added to Heroku
// todo: :4200 needs to be added if you want to run from Angular client in dev after jar
...
heroku config:set SPRING_DATASOURCE_URL=jdbc:postgres://<value-after-@-from-last-command> --remote jib
...
----
// todo: this doesn't work. It did when I first did it a couple weeks ago. WTF? This blog post says SPRING_DATASOURCE_* variables will be auto-created when you add PostgreSQL https://devcenter.heroku.com/articles/deploying-spring-boot-apps-to-heroku.

NOTE: This fine-grained configuration is not necessary when Heroku knows you have a Spring Boot app. In this case, it's oblivious to your app's architecture since it's running as a container.
...
----
// todo: talk to Joe and see if it's possible to deploy an image if it's not built locally. It's a real pain to upload twice on a slow connection!
...
Since the Okta addon for Heroku configures everything for you, you should be able to open your app, click the **Login** button, and authenticate!
...
EOF
----
// todo: is it possible to set SPRING_JPA_HIBERNATE_DDL_AUTO=validate w/o specifying all the other variables.
...
// todo: how the fuck do you find out this URL? It seems like i have it deployed, but `kubectl get pods` only shows postgres running.
...
----
// todo: Is this necessary? When I run it, I get "Server error, status code: 403, error code: 10003, message: You are not authorized to perform the requested action"
...
cf restage $APP_NAME
----
// todo: how the hell do you get the postgresql URL from Cloud Foundry?
...
The built-in Docker support provided by Spring Boot provides a great way to get started with buildpacks. Since it's an implementation of the buildpack platform specification, it's also easy to migrate to more powerful buildpack tools such as https://github.com/buildpacks/pack[`pack`] or https://github.com/pivotal/kpack[`kpack`].
// todo: confirm with Joe

// todo: add a blurb about JHipster and Docker
