...
type: conversion
changelog:
- 2021-09-15: Updated to use Micronaut 3.0.1, Quarkus 2.2.2.Final, and Spring Boot 2.5.4. See this post's changes in [okta-blog#885](https://github.com/oktadev/okta-blog/pull/885) and the example app changes in [native-java-examples#6](https://github.com/oktadev/native-java-examples/pull/6).
---
:page-liquid:
:toc: macro
:experimental:
...
You can see it starts pretty darn quick (23ms)! Test it with HTTPie and an access token. You may have to generate a new JWT with https://oidcdebugger.com[oidcdebugger.com] if yours has expired.
...
The native compilation step will take a bit to complete. On my 2019 MacBook Pro, it took 1 min. 23 s.
...
2021-09-15 09:42:35,854 INFO  [io.quarkus] (main) quarkus 1.0.0-SNAPSHOT native (powered by Quarkus 2.2.2.Final) started in 0.023s. Listening on: http://0.0.0.0:8080
...
Supersonic Subatomic Java (in 23ms)! Test it with HTTPie and an access token.
...
But wait, doesn't Okta link:/blog/2020/11/24/spring-boot-okta[have a Spring Boot starter]? Yes, we do! When this post was first written, it https://github.com/okta/okta-spring-boot/issues/192[didn't work with GraalVM yet]. This has been fixed in the v2.2.0 release. See https://github.com/oktadev/native-java-examples/pull/5[this pull request] for the required changes.

We've also published a /blog/2021/09/16/spring-native-okta-starter[Spring Native in Action] blog post. It contains the video from our Twitch stream with https://twitter.com/starbuxman[Josh Long]. You can watch it to see what we discovered and how we fixed it. Since then, Spring Native has improved things so you only need to enable HTTPS!
...
The native compilation step will take a bit to complete. On my 2019 MacBook Pro, it took 3 min. 44 s.

Start it using Docker:

----
$ docker run -p 8080:8080 demo:0.0.1-SNAPSHOT
2021-09-15 16:30:25.496  INFO 1 --- [           main] o.s.nativex.NativeListener               : This application is bootstrapped with code generated with Spring AOT
...
( ( )\___ | '_ | '_| | '_ \/ _` | \ \ \ \
 ...
 :: Spring Boot ::                (v2.5.4)
...
2021-09-15 16:30:25.588  INFO 1 --- [           main] o.s.b.w.embedded.tomcat.TomcatWebServer  : Tomcat started on port(s): 8080 (http) with context path ''
2021-09-15 16:30:25.589  INFO 1 --- [           main] com.okta.rest.DemoApplication            : Started DemoApplication in 0.102 seconds (JVM running for 0.104)
----
