...
|Firebase, https://npmjs.org/package/@angular/fire[`@angular/fire`]
|Azure, https://npmjs.org/package/@azure/ng-deploy[`@azure/ng-deploy`]
|Now, https://npmjs.org/package/@zeit/ng-deploy[`@zeit/ng-deploy`]
|Netlify, https://www.npmjs.com/package/@netlify-builder/deploy[`@netlify-builder/deploy`]
|GitHub pages, https://npmjs.org/package/angular-cli-ghpages[`angular-cli-ghpages`]
|NPM, https://npmjs.org/package/ngx-deploy-npm[`ngx-deploy-npm`]
|AWS S3, https://www.npmjs.com/package/@jefiozie/ngx-aws-deploy[`@jefiozie/ngx-aws-deploy`]
|===
...
Microsoft Azure is a cloud computing service from Microsoft. I've never used it before writing this post, but I sure hope it's easier than AWS!

Check out your `master` branch and create a new `azure` one.
...
Add the Azure deployment plugin for Angular:
...
This process gave me a URL to login to Microsoft. I have a Microsoft account and I was able to use that to sign-in. After I did, I was prompted to https://azure.com/free[create a free Azure account].

After creating my account (which required a phone number _and_ a credit card), I tried installing `@azure/ng-deploy` again.

I received the same error message about not having any active subscriptions. I clicked around the Azure portal a bunch, and ended up upgrading my account to a pay-as-you-go subscription. I chose **No technical support** because it was free. The Developer plan is $29/month at the time of this writing.

When the `ng add` process failed again, I decided to give up. The `@azure/ng-deploy` schematic failed my 10 minute test.
...
TIP: When I was working on this file, I found it handy to comment out the `ng build` line and use `RUN mkdir -p dist/notes && echo "Hello, World" > dist/notes/index.html` instead to skip the lengthy Angular build process.
...
If you find these `docker` commands hard to remember like I did, add a couple scripts to your `package.json` file.
...
IMPORTANT: One advantage of doing this is you can run your local Angular app against your production backend. I've found this very useful when debugging and fixing UI issues caused by production data.
