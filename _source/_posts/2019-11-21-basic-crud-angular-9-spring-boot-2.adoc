...
url: basic-crud-angular-9-spring-boot-2
---
:toc: macro
:page-liquid:
:experimental:
...
[source,shell]
----
npm install -g @angular/cli@9.0.0-rc.5
----
...
To see how your final `DemoApplication` file should look, you can [view the completed version in GitHub](todo).
...
The latest version has a number of improvements and hasn't been published, so you have to clone it from GitHub, install, and link to it.

[source,shell]
----
git clone https://github.com/manfredsteyer/angular-crud.git
cd angular-crud/angular-crud
npm install
npm link
----

Then, in your Angular `notes` app, install `angular-crud` using npm:

[source,shell]
----
npm link angular-crud
//npm i -D angular-crud
----

In your Angular `notes` app, create a `src/app/note` directory.
...
CREATE src/app/note/note.service.ts (2064 bytes)
...
You can use https://github.com/FiloSottile/mkcert[mkcert] for to generate local, valid TLS certificates. To force HTTPS, you just need to configure Spring Security. I like to just do it in production so I don't need to install certificates in development.
...
Angular's `HttpClient` has built-in support for the client-side half of the CSRF protection. It'll read the cookie sent by Spring Boot and return it in a `X-XSRF-TOKEN` header. You can read more about this in https://angular.io/guide/security[Angular's Security docs].
...
This tutorial did not show you how to make your Angular app look good or how to deploy it to a public server. I'll tackle those topics in an upcoming post. In the meantime, you can see a previous tutorial I wrote to see how this app might look with Angular Material.
...
You can find the source code for the completed application at https://github.com/oktadeveloper/okta-spring-boot-2-angular-9-example[oktadeveloper/okta-spring-boot-2-angular9-example].

This blog has a bunch of Angular, Spring Boot, and Kotlin tutorials. Here are some I recommend:

* // todo: find top 2 angular posts and list them here
...
If you like this article and want to see more like it, follow @oktadev on Twitter, subscribe to [our YouTube channel], or follow us on LinkedIn. As always, please leave a comment below if you have any questions.
