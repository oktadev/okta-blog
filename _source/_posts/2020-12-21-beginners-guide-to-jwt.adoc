...
When an Okta authorization server returns an access or ID token to a client, two claims are of particular note:
 ...
 - Audience (`aud`) - A list of parties the token should be sent to and parsed by. The audience is the Okta Authorization Server Issuer URI for access tokens requesting Okta API access. The audience is the customer's API URI for access tokens requesting customer API access. For ID tokens, it's the client ID. Working with Okta, only one server is being targeted, so the list should only contain one item.
