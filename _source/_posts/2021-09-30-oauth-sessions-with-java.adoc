...
- "Confused how #OAuth2 and affects your web app's sessions? Learn more and build an example with #Redis, #HAproxy, and #SpringBoot 💡"
...
A common OAuth 2.0 question we hear is: "How do I deal with OAuth with a load-balanced application". The short answer to the question is there is nothing specific about session clustering for OAuth. The longer answer is, you likely still need to worry about cluster session management. This post will discuss how an OAuth login relates to your application's session and build a simple, secure, load-balanced application.
...
1. User requests to log in - automatically when requesting a protected page or when the user clicks on a login button.
...
5. Issue redirect with a code back to Web App.
...
At this point, you might be wondering if you could cram everything in a JWT and make this process stateless; removing the need for any session clustering. You _could_, but you would need to use a JWE (JSON Web Encryption) to ensure the browser didn't have access to any sensitive data. It is outside the scope of this post, but there are a whole host of reasons link:/blog/2017/08/17/why-jwts-suck-as-session-tokens[not to use JWTs as session tokens].

== Load Balancer Sticky Sessions

Another option to avoid session replication or clustering is to use "sticky sessions" (or "session affinity"), but this creates a fragile environment. If a web server fails or is taken out down for any reason, all users associated with that server would be essentially be logged out. Sticky sessions also violate the https://12factor.net/processes[12 Factor principles of process statelessness].

IMPORTANT: Don't confuse 12 Factor's reference of "stateless" with the previous section; this specifically refers to the need for an application relay on the "state" of a server between requests, for example data cached in memory or a file. Instead, a https://12factor.net/backing-services[backing service] should be used; in the case of session storage, the example of Redis used, which is exactly what we are going to do in the next section.
...
HAproxy is used to distribute requests between multiple `backend` applications, create a configuration file, `haproxy.cfg`, that will serve two different web apps (one on port `8081`, and another on `8082`).
...
You can spot the problem if you think back to the sequence diagram at the beginning of this post. The initial login request (step one) happened on one instance, and the final one occurred on another (step six). Updating the application to use shared session storage will fix the problem.

Let's do that!

== Share Sessions with Redis

If you are following along, you already have a Redis server running; all that's left is to configure the Spring Boot application to make use of it. Luckily, https://spring.io/projects/spring-session[Spring Session] makes the process painless.
...
WARNING: By defaultSpring Session configures Redis to connect to `localhost` on port `6379` and NO password, take a look at the https://docs.spring.io/spring-session/docs/current/reference/html5/guides/boot-redis.html#boot-redis-configuration[Spring Session documentation] for various configuration options.
...
Refresh the browser a few times you will see the response alternating between the ports:
...
This post has shown how to manage sessions for a simple load-balanced application that uses OAuth 2.0. The example used focused on the Spring Boot portion of the application, but it's far from production-ready; a few other concerns would need to be addressed:
...
If you liked this post, check out this related content:
