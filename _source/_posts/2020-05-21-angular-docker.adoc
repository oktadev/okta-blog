...
image::{% asset_path 'blog/angular-docker/nginx-headers.png' %}[alt=Nginx in Docker score from securityheaders.com,width=800,align=center]
...
// todo: Ask Ray if it's worth mentioning https://skaffold.dev/
...
To begin, https://buildpacks.io/docs/install-pack/[install `pack`]. If you're on a Mac or Linux, you can use Homebrew. If you're on Windows, you can [install its executable](https://github.com/buildpacks/pack/releases/download/v0.10.0/pack-v0.10.0-windows.zip).
...
When you deployed Spring Boot to Heroku earlier, you used Heroku's Gradle buildpack. You can use it with Cloud Native Buildpacks too!

You'll need to include `heroku/nodejs` along with `heroku/java` because the Gradle process calls Node to build Angular.

Run the following command to create an image:

[source,shell]
----
pack build mraible/bootiful-angular \
  --builder heroku/buildpacks \
  --buildpack heroku/java,heroku/nodejs \
  --env GRADLE_TASK='bootJar -Pprod'
----

If you setup the Docker Compose `.env` file and symlinked it in the `notes-api` directory, you should be able to run this image and login at `http://localhost:8080`.
...
Using `pack` is pretty cool, but the command is really long, and difficult to memorize. Spring Boot 2.3 to the rescue!
...
// todo: Make sure Spring Boot is already upgraded to 2.3
...
image::{% asset_path 'blog/angular-docker/spring-boot-2.3-notes-noshadow.png' %}[alt=Spring Boot 2.3 app running,width=800,align=center]
...
* Angular + `Dockerfile`
* Angular and Spring Boot in a JAR
* Docker images with Jib
* Docker images with Buildpacks

You can find the source code for this example on GitHub at https://github.com/oktadeveloper/okta-angular-docker-example[oktadeveloper/okta-angular-docker-example].
