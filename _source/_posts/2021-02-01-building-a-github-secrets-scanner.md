...
We will be working with Python, the GitHub REST API, and the GitPython Library. You will need to obtain a [GitHub personal access token](https://github.com/settings/tokens) and [install GitPython](https://gitpython.readthedocs.io/en/stable/intro.html#installing-gitpython). 
...
Before we dive into the code, here are the imports you need:
```python
...
First, we will crawl through your organization’s account to find repositories to analyze. We need the GitHub API for this. The GitHub API endpoint is located at `https://api.github.com`.
You can set the “Accept” header to “application/vnd.github.v3+json” to specify that we are using the REST API v3. And you can provide the access token through an “Authorization” header.
```python
...
def find_contributors(org, repo):
    ...
    for user in resp:
        users.append(user["login"])
```
Next, we scan the usernames for repositories and clone the repo into a temporary directory for analysis. You might have to figure out some permission issues with the GitHub API during this step. Make sure that your access tokens have the proper permissions!
```python
def find_repos():
...
Now, let’s start scanning our repositories for keywords! We can use the GitPython library to iterate through the commits of each repository and scan for the strings in our dictionary. Finally, we display the filenames and commit numbers that might be leaking information.
```python
class colors:
    FILENAME = "\033[1m"
    NORMAL = "\033[0m"
...
In this post, we built a simple GitHub scanner that finds secrets accidentally committed to your organization’s repositories. If this is something that interests you, here are some related project ideas.
