...
layout: blog_post
title: "Spring Native: start your JHipster + Spring Boot app in just 1s️!"
#title: "Spring Native: starting your Java apps in seconds!"
#title: "Introducing JHipster Native: serverless full-stack Java for all ⚡️"
...
As a result of Josh and I's successful https://youtu.be/F9oydL_MndA[presentation at the SF JUG], we were invited to speak at the Garden State JUG in December. We laid low for a few months and rekindled our research in December. I wrote about it in https://www.linkedin.com/pulse/jhipster-works-spring-native-part-2-matt-raible/[JHipster works with Spring Native, Part 2!]

== 🔥 Announcing the JHipster Native blueprint v1.0!

In late January 2022, we (the JHipster team) upgraded JHipster's to use Spring Boot 2.6 and released version https://www.jhipster.tech/2022/01/23/jhipster-release-7.6.0.html[7.6.0].
...
This was on a Friday afternoon (mountain time). By the next morning, Marcelo had the MVP finished and provided steps to reproduce our examples. 😳
...
[TIP]
====
Want results? Clone the `spring-native` branch with the changes below already made:
----
...
image::{% asset_path 'blog/spring-native-jhipster/commit-tools-window.png' %}[alt=IntelliJ IDEA's Commit Tools Window,align=center]
...
The `@NativeHint` solves the following exception that happens when you upload a photo:
...
To switch from Keycloak to Auth0, you only need to override the Spring Security OAuth properties. You don't even need to write any code!
...
Press the **Create Application** button in https://manage.auth0.com/#/applications[Applications section]. Use a name like `JHipster Native!`, select `Regular Web Applications`, and click **Create**.
...
The JHipster Native plugin integrates Spring Native into a JHipster project based on https://github.com/mraible/spring-native-examples#readme[the findings from Josh Long and my research]. I documented our findings in September and December 2021.
...
The crazy thing is, one of the hardest problems we had to solve was with JPA and relationships. In JVM mode, everything worked fine. When running in native mode, https://github.com/jhipster/generator-jhipster/issues/17794[there was an exception]. The solution took days to figure out, but was quite simple to fix: I just had to add a type hint for `java.util.HashSet.class`. 🤯

During this experience, the only major lacking thing I found lacking is Spring Native https://github.com/spring-projects-experimental/spring-native/issues/465[doesn't support caching yet]. I believe this support will be added by the community soon. In the meantime, if you're looking to start/stop your infra as fast as possible, you probably don't care about caching. Caching is made for long-lived, JVM-strong, JVM-loving apps.
