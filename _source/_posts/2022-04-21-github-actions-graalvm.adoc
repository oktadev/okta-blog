...
If you do, every time you create a PR, and commit to it, it will run a native build. A GraalVM build of this project takes 3-4 minutes for me locally. With GitHub Actions, it takes 30+ minutes! Also, it only builds on Linux, so you won't know if there are issues with macOS or Windows.
...
When I first started investigating GitHub Actions for GraalVM, the JHipster Native blueprint modified commands in `package.json` to always build native images and to use them when running end-to-end tests. This meant that when you first tried to add GitHub Actions support, the build would fail because `GRAALVM_HOME` wasn't found. To solve this, you could switch from `actions/setup-java` to `graalvm/setup-graalvm` to solve this, but that's not very environmentally sustainable.
...
1. `native.yml`: run nightly tests using GraalVM
...
When I first tried triggering it, it failed with the following error:

----
The job running on runner GitHub Actions 2 has exceeded the maximum execution time of 40 minutes.
----

I'll address this issue in a bit.

== How to build and upload native binaries when releasing on GitHub

The `native-artifact.yml` workflow file creates binaries for macOS, Linux, and Windows when a release is created. This file shows how to configure Linux and Windows so they have enough memory, uploads artifacts to the actions job, and uploads the native binaries to the release on GitHub. It will only execute when you create a release (aka a tag).
...
Commit it to the main branch and push.
