...
Project Helidon is a set of Java libraries for writing microservices. It's an open source, Apache 2.0-licensed, Oracle-sponsored project and contains support for MicroProfile as well as a reactive, functional API. Helidon's two different style are known as Helidon MP and SE, respectively. The https://helidon.io[helidon.io] website says:
...
Content-Length: 0
Date: Fri, 3 Dec 2021 06:11:25 -0700
...
image::{% asset_path 'blog/native-java-helidon/httpie-helidon-bearer-token.jpg' %}[alt=HTTPie call to Helidon's /hello with bearer token,width=800,align=center]
...
This command will take a few minutes to complete. My 2019 MacBook Pro with a 2.4 GHz 8-Core Intel Core i9 processor and 64 GB of RAM took 2 min. 34 s. to finish.
...
2021.12.03 06:18:13 INFO io.helidon.microprofile.server.ServerCdiExtension Thread[main,5,main]: Server started on http://localhost:8080 (and all other host addresses) in 550 milliseconds (since JVM startup).
2021.12.03 06:18:13 INFO io.helidon.common.HelidonFeatures Thread[features-thread,5,main]: Helidon MP 2.4.0 features: [CDI, Config, Fault Tolerance, Health, JAX-RS, Metrics, Open API, REST Client, Security, Server, Tracing, Web Client]
2021.12.03 06:18:13 INFO io.helidon.common.HelidonFeatures.experimental Thread[features-thread,5,main]: You are using experimental features. These APIs may change, please follow changelog!
2021.12.03 06:18:13 INFO io.helidon.common.HelidonFeatures.experimental Thread[features-thread,5,main]: 	Experimental feature: Web Client (WebClient)
----

You can see it starts in a just over half a second (550ms)! Test it with HTTPie and an access token. You may have to generate a new JWT with https://oidcdebugger.com[oidcdebugger.com] if yours has expired.
...
+
image::{% asset_path 'blog/native-java-helidon/helidon-init.jpg' %}[alt=helidon init command,width=800,align=center]
...
. Add a `HelloApplication` class in `src/main/java/com/okta/rest/HelloApplication.java` to register your resource and configure JWT authentication:
+
[source,java]
----
...
_These numbers are from a 2019 MacBook Pro with a 2.4 GHz 8-Core Intel Core i9 processor and 64 GB of RAM. I think it's important to note that my WiFi connection was 71 Mbps down and 10 Mbps up (according to the Speedtest app)._
