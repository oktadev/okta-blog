...
[comment]
--
todo: research and figure out best title. Other options:
...
Open another terminal and navigate to the `ionic-app` folder. Launch your Ionic client using  `npm start`. The app will be opened at `\http://localhost:8100` in your default browser.
...
When you save this file, the Ionic CLI will auto-compile and reload the app in your browser. You can prove everything works as expected by stopping your app (with kbd:[Ctrl + C]) and running all the end-to-end tests with Cypress.
...
With Keycloak, you don't need a separate OIDC app for Ionic. With Okta, you do. See JHipster's documentation to learn https://www.jhipster.tech/security/#create-a-native-app-for-mobile-on-okta[how to create a native app for mobile on Okta].
...
Next, https://www.jhipster.tech/security/#create-a-native-app-for-mobile-on-auth0[configure a native app for mobile on Auth0]. Once you're finished updating your Ionic app with a new client ID and audience, you should be able to run your backend and new frontend client using the following commands:
