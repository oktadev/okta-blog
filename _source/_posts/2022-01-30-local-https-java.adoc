...
Create a properties file `~/.config/spring-boot/spring-boot-devtools.properties` containing the server port and newly generated key store location.
...
It's not all sunshine and roses, there are a few things to watch out for. If you are a developer that works on multiple applications.
If you are switching between applications that run on HTTP and HTTPS prototypes, your browser may automatically switch to HTTPS, especially one of your applications sets the https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Strict-Transport-Security[HTTP Strict-Transport-Security (HTST) headers]. If you run into this type of problem you can clear the https://www.a2hosting.com/kb/getting-started-guide/internet-and-networking/clearing-a-web-browsers-ssl-state[SSL State] and/or https://www.thesslstore.com/blog/clear-hsts-settings-chrome-firefox/[HSTS settings] in your browser.

TIP: Use different hostname for each application you are working on to avoid any browser caching.
