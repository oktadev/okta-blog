...
In the fall of 2020, the Spring team released a new experimental project that gave Spring developers hope. It was called Spring Native. Spring Native is all about converting your Spring applications to native executables. It leverages GraalVM to make it happen. This was huge because the new kids on the block, Micronaut and Quarkus, produced native executables by default.

I was really excited about Spring Native when I first heard about it. It's first release (v0.8.3) was on Nov 23, 2020, but I started playing with it in September 2020, probably as a result of SpringOne. How do I know this? Because that's when I first tried it and [entered an issue for the Okta Spring Boot starter](https://github.com/okta/okta-spring-boot/issues/192). I was able to get things to working with Spring Security's OAuth dependencies (instead of the Okta starter) in the waning weeks of 2020.
...
This stream originally aired on June 22, 2021. Since then, there's been a few Spring Boot releases, as well as several Spring Native releases. And don't forget, there was another SpringOne!

== Okta Spring Boot Starter v2.2 Supports Spring Native!

Today, I'm happy to announce that we've added our learnings to the Okta Spring Boot starter v2.2.0; now it only takes a couple lines to add native support to a Spring Boot application that uses Okta.
...
You can also try it out yourself with the Okta CLI.
...
Add `@NativeHint(options = "--enable-https")` to the `Application` class as specified above. If it's already there, that means we decided to add it by default! ð
