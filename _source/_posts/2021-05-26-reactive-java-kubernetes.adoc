...
CAUTION: Netflix Zuul is no longer supported by Spring Cloud. There is an https://github.com/spring-cloud/spring-cloud-gateway/issues/36[open issue] to add Spring MVC/Servlet support to Spring Cloud Gateway. It's scheduled to be implemented before the end of 2021.
...
I could show you how to get everything working with Docker Compose, but I already showed you link:/blog/2021/01/20/reactive-java-microservices#run-your-microservices-stack-with-docker-compose[how to do that in the previous tutorial]. Today, I'd like to show you how to run things locally with https://minikube.sigs.k8s.io/docs/[Minikube].
...
[source,shell]
----
minikube --memory 20000 --cpus 10 start
----

This command will start Minikube with 20 GB of RAM and 10 CPUs. The default, 16 GB RAM and two CPUS, did not work for me.
...
TIP: You can stop Minikube with `minikube stop` and start all over with `minikube delete`.
...
You can see if everything start up using the following command.
...
You can also automate testing that everything works. Set your Okta credentials as environment variables and run end-to-end tests using Cypress (from the gateway directory).
...
You might notice I used a secret in plain text in the `application-configmap.yml` file. This is a bad practice! I hope you didn't check everything into source control yet!!
...
Then, copy the encrypted value back in to `application-configmap.yml`. Make sure to wrap it in quotes!
...
TIP: If you want to make it so you don't need to restart the Spring Cloud Config server when you update its configuration, see link:/blog/2020/12/07/spring-cloud-config#refresh-the-configuration-in-your-spring-cloud-config-server[Refresh the Configuration in Your Spring Cloud Config Server].
...
You created Docker images earlier to run with Minikube. Those images were deployed to Docker Hub or your local Docker registry. If you deployed to Docker Hub, you can use your deployment files as-is.
...
You can monitor the progress of your deployments with `kubectl get pods -n default`.

[TIP]
====
If you mess up the configuration for JHipster Registry and need to deploy it, you can do it with the following commands.

[source,shell]
----
kubectl delete pod jhipster-registry-0 -n default
kubectl delete pod jhipster-registry-1 -n default
kubectl apply -f registry-k8s/jhipster-registry.yml
----
...
[source,shell]
----
EXTERNAL_IP=$(kubectl get svc gateway -n default -ojsonpath="{.status.loadBalancer.ingress[0].ip}")
...
[source,shell]
----
kubectl apply -f certificate.yml -n default
kubectl apply -f ingress.yml -n default
----
...
[source,shell]
----
kubectl describe managedcertificate gateway-certificate -n default
----
...
[source,shell]
----
kubectl scale deployments/store --replicas=2 -n default
----
...
To install it on macOS, run `brew install k9s`. Then run `k9s` to start it. You can navigate to your pods, select them with kbd:[Return], and navigate back up with kbd:[Esc].
