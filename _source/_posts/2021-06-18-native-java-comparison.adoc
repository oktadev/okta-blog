...
NOTE: If you're not sure what OIDC and OAuth 2.0 are, see our https://developer.okta.com/blog/2019/10/21/illustrated-guide-to-oauth-and-oidc[Illustrated Guide to OAuth and OpenID Connect].
...
This command will take a few minutes to complete. My 2019 MacBook Pro with a 2.4 GHz 8-Core Intel Core i9 processor and 64 GB of RAM took 3 min. 7 s to finish.
...
The native compilation step will take a bit to complete. On my 2019 MacBook Pro, it took 2 min. 27 s.
...
Supersonic Subatomic Java (in 31ms)! Test it with HTTPie and an access token.
...
But wait, doesn't Okta https://developer.okta.com/blog/2020/11/24/spring-boot-okta[have a Spring Boot starter]? Yes, we do—however, it https://github.com/okta/okta-spring-boot/issues/192[doesn't work with GraalVM yet].
...
2021-06-16 02:21:24.971  INFO 1 --- [           main] com.okta.rest.DemoApplication            : Started DemoApplication in 0.824 seconds (JVM running for 0.794)
----
...
5. Enable HTTPS for native builds by adding a `BP_BOOT_NATIVE_IMAGE_BUILD_ARGUMENTS` property to the Spring Boot Maven plugin.
+
[source,xml]
----
<plugin>
...
_These numbers are from a 2019 MacBook Pro with a 2.4 GHz 8-Core Intel Core i9 processor and 64 GB of RAM._

////
Micronaut: (26 + 29 + 25 + 27 + 27) / 5 = 26.8ms
Quarkus: (18 + 24 + 18 + 18 + 18) / 5 = 19.2ms
Spring Boot (549 + 611 + 568 + 763 + 858) / 5 = 669.8ms
////
...
|Micronaut | `./target/app` | 26.8
|Quarkus | `./target/quarkus-1.0.0-SNAPSHOT-runner` | 19.2
|Spring Boot | `docker run -p 8080:8080 docker.io/library/demo:0.0.1-SNAPSHOT` | 669.8
|===
...
The Spring Boot startup times looked a little long, so I contacted my friend https://twitter.com/starbuxman[Josh Long]. We did a debugging session over Zoom and discovered the longer startup times is because Spring Security is doing OIDC discovery with the issuer.

Spring Boot's "initialization completed" time seemed to be right around 64ms. The duration between that and the "Started in ..." time is the time it takes to make the call to Okta. We tried optimizing it by just using the JWKS URI. For example:
...
This did improve things quite a bit.

// (.136 + .183 + .145 + .154 + 0.159) / 5 = 155.4
++++
<div id="spring-boot-jwks"></div>
<script>
google.charts.load('current', {packages: ['corechart', 'bar']});
google.charts.setOnLoadCallback(drawChart);
...
function drawChart() {
  ...
  var chart = new google.visualization.BarChart(document.getElementById('spring-boot-jwks'));
...
When I changed it to a Keycloak instance generated by JHipster, the times were much improved. Josh showed me how he was able to improve startup performance even more by creating a `application-cloud.properties` that sets Keycloak's hostname to `host.docker.internal`. I watched in awe as he achieved 10s startup times.
...
When building native images, it's essential to test them as part of an integration testing process. This post is already long enough, so I won't explain how to test native images in this post.

// We'll publish a post in the future that covers this topic.
...
You can find the source code for all the examples used in this post https://github.com/oktadev/native-java-examples[on GitHub].
...
- https://developer.okta.com/blog/2019/11/27/graalvm-java-binaries[Watch GraalVM Turn Your Java Into Binaries]
- https://developer.okta.com/blog/2020/01/09/java-rest-api-showdown[Java REST API Showdown: Which is the Best Framework on the Market?]
