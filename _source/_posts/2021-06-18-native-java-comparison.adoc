...
TIP: You might want to keep OpenID Connect <debugger/> open to copy your access tokens. It allows you to easily start over and regenerate a new access token if it expires.
...
This command will take a few minutes to complete. On my 2019 MacBook Pro with a 2.4 GHz 8-Core Intel Core i9 processor and 64 GB of RAM, it took 3 min. 7 s.
...
I'm not certain these startup times are apples to oranges. It seems kind of strange that Spring Boot takes so much longer. Maybe it's because it's started with Docker?

== Testing Native Images

When building native images, it's important to test them as part of integration testing process. This post is already long enough, so I won't explain how to test native images in this post. We'll publish a post in the future that covers this topic.
...
At the beginning, I mentioned JNI and JNA. Baeldung has some tutorials about both:
