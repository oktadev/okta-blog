...
When I changed it to a Keycloak instance generated by JHipster, the times improved quite a bit. Josh showed me how he was able to improve startup performance even more by creating a `application-cloud.properties` that sets Keycloak's hostname to `host.docker.internal`. I watched in awe as he achieved 10s startup times.
