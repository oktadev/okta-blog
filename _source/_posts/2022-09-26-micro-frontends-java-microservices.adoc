...
It also generates files for Kustomize and Skaffold, if you'd prefer to use those tools.

I won't go into the nitty-gritty details of deploying a JHipster microservices stack to cloud providers with K8s, mostly because it's covered in previous blog posts. The first post below shows how to run Minikube locally, as well as deploy to Google Cloud.
