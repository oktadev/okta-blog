...
This project uses Gretty plugin version 4.0.3 which uses Jetty version 11.0.11.  Since version 4, Gretty requires Jakarta over Javax. You can use Gretty 3 if you want to support non-Jakarta apps (this is explained on the Gretty GitHub page listed above).
...
The `build.gradle` file, which is analogous to the `pom.xml` file.
...
This is about as simple as it gets. You can see the same Jakarta-based servlet API being included as a provided dependency as well as the Gretty plugin being included in the `plugins` block.
...
You can also take a look at the Gradle tasks that the plugin adds [in the docs](https://gretty-gradle-plugin.github.io/gretty-doc/Gretty-tasks.html). 
