...
An example of simplifying existing capabilities is how to create components dynamically. Dynamic components are components loaded at runtime. You might use this when you need to tailor visible components based on responses from a server call. Another example might be if your identity token includes a custom claim, such as department, and you want to show a custom dynamic component specific to departmental information. 

Previously, to dynamically create components, you had to use the `ComponentFactoryResolver`. Now the API takes the Component type directly. Let's look at an example.

In my example, I added a custom claim using the Okta dashboard named `pet_preference`. The app uses the claim value to rotate through images of pets that match their preference.
...
Creating Angular apps is now easier with enhancements to developer tooling, compilation time improvements, and updates to testbed environments.
...
{% img blog/angular-v13/auto-apply-optional-chaining.gif alt:"Auto applying optional chaining in VS Code for a deep object with optional properties" width:"400" %}{: .center-image }
