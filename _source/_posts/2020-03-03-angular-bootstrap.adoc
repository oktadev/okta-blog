...
// angular-bootstrap.png here [alt=Angular 💜 Bootstrap,width=800,align=center]

toc::[]

== Isn't Angular Dead?

I've heard many developers say that Angular is Dead. As a veteran Java developer, I've heard this said about Java many times over the years as well. Yet it continues to thrive. Angular is similar in that it's become somewhat boring. Some people call boring frameworks "legacy". Others call it "revenue-generating".
...
To make the login button work, you need to configure both app's security configurations.

== Secure Angular + Bootstrap with OpenID Connect
...
Create a `okta.env` file in the `notes-api` directory and copy your settings into it.
...
3. Give the application a name, set the login redirect URI to `http://localhost:4200/implicit/callback`
4. Click **Done**
...
Now you should be able to log in with your Okta credentials and create notes to your hearts content!
...
You'll see there's three files that have a `.css` extension.
...
And change `styles.scss` to use import Bootstrap's Sass.
...
However, if you click on it, the menu doesn't expand. To fix that, you need to use the `ngbCollapse` directive from NG Bootstrap. Modify `app.component.html` to have a click handler on the navbar toggler and add `ngbCollapse` to the menu.
...
Then add `isCollapsible` in `app.component.ts` and and change the title to be capitalized.
...
Modify the `note-list.component.html` to so the breadcrumb doesn't float right and all the content is in the same card.
...
Now when you add a new note, it'll let you know that the name is required.
...
In the beginning of this tutorial, I said I'd show you how to develop a searchable, sortable, and pageable data table. NG Bootstrap has a https://ng-bootstrap.github.io/#/components/table/examples#complete[complete example] I used to build the section below. The major difference is you'll be using a real server, not a simulated one. Spring Data JPA has some slick features that make this possible, namely its query methods and paging/sorting.

=== Add Search by Title with Spring Data JPA

Adding search functionality requires the least amount of code modifications. Change the `UserController#notes()` method in your Spring Boot app to accept a title parameter and return notes with the parameter's value in their title.
...
interface NotesRepository : JpaRepository<Note, Long> {
    ...
    fun findAllByUserAndTitleContainingIgnoringCase(name: String, term: String): List<Note>
}
----

Restart your server, add a few notes, and you should be able to search for them by title in your Angular app. I love how Spring Data JPA makes this so easy!
...
Update `note-list.component.html` so it uses the `sortable` directive and calls `onSort()` when clicked.
...
Add CSS in `styles.scss` to show a sort indicator when a column is sorted.
...
Restart your Spring Boot app so the data is available for searching. Click on the **Title** column to see sorting in action!
...
In `note-list.component.ts`, add a `total$` observable and set it from the `search()` method. You'll also need to add an `onPageChange()` method, an `onChange()` method, and modify `onSort()` to set the page to 0.
...
Then update `notes.service.ts` to add add a `size$` observable and parameters for the page size and page number.
...
Now your note list should have a working pagination feature at the bottom. Petty slick, eh?
...
2. Create a `easy-notes` directory and a `notes.jdl` file in it
...
Of course, you probably want to see it running. Run the following commands to start Keycloak (as a local OAuth 2.0 server) and start the app.
...
I used the following resources to gather the historical information about Angular and Bootstrap.
...
* https://developer.okta.com/blog/2020/01/06/crud-angular-9-spring-boot-2[Build a CRUD App with Angular 9 and Spring Boot 2.2]
* https://developer.okta.com/blog/2020/02/12/angular-ivy[What Is Angular Ivy and Why Is It Awesome?]
* https://developer.okta.com/blog/2020/01/21/angular-material-login[Build a Beautiful App + Login with Angular Material]
* https://developer.okta.com/blog/2020/01/31/java-graphql[How to GraphQL in Java]
* https://developer.okta.com/blog/2019/04/04/java-11-java-12-jhipster-oidc[Better, Faster, Lighter Java with Java 12 and JHipster 6]
