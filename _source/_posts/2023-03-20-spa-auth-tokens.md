...
tweets:
- "Let's demystify authentication and authorization in single-page application with this step-by-step explanation!"
...
The client application kicks off the process and generates a random, long string — the code verifier. Then it creates a code challenge from the code verifier. This step is part of the PKCE extension.
...
{% img blog/spa-auth-tokens/flow-3-authorization-code-request.jpg alt:"The client application making an authorization code request to the authorization server, Okta, with the above mentioned information" width:"800" %}{: .center-image }
...
If you haven't already done so, open the SPA code in your favorite IDE and navigate to the `config.js` or `app.config.ts` file, depending on your framework. You'll see the OIDC config and a property for `scopes` where you will add `offline_access`. Your OIDC config looks something like this:
...
Now that we have these tokens in our SPA, what do we do with them? We should let a trusted OIDC library, such as the Okta SDKs we're using, handle all the token requests and refresh them for us. Behind the scenes the OIDC library is hard at work exchanging tokens.
