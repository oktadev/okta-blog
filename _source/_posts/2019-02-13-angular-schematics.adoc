...
type: conversion
changelog:
- 2021-04-14: Updated to use Angular 11 and OktaDev Schematics v3.4.1. You can see the changes to this post in [okta-blog#707](https://github.com/oktadeveloper/okta-blog/pull/707); example app changes are in [okta-angular-schematics-example#1]().
---
:toc: macro
:page-liquid:
...
This will prompt you for an OIDC issuer and client ID.

{% include setup/cli.md type="spa" framework="Angular" loginRedirectUri="http://localhost:4200/callback" %}

If you haven't run the `ng add @oktadev/schematics` command, do it now. For the issuer URL and client ID, use the values from the app you just created. You can see a screenshot of my values and the process below.
