...
description: "Use Okta + Oso to Secure a FastAPI + SQLAlchemy App"
tags: []
tweets:
- ""
- ""
...
Next, we'll want to perform _authorization_, which controls **what** the
...
First, either log in to your existing Okta account or create a new one via
[this link][Okta signup].

Next, follow [these instructions][Okta create app] to create a new Okta
application configured for 'Single-Page App' mode. On the 'Application
Settings' screen, make sure to update the 'Login redirect URIs' value from the
default `http://localhost:8080/implicit/callback` to
`http://localhost:8080/login/callback`. Click 'Done' to create your new Okta
app.

Stay on the ensuing screen for a moment — we're going to need the 'Client ID'
and 'Okta domain' properties for your new app. Create a file named `.env` in
the `okta-hosted-login` directory, and add the following two lines:

```ini
CLIENT_ID=<Your Client ID here.>
ISSUER=https://<Your Okta domain here.>/oauth2/default
```
...
Once the app is up and running, navigate to [localhost:8080][] in your browser.
...
If you reload [localhost:8080][], you should see the list of bears populated
...
authorization system that we'll use to secure our app. Oso is flexible enough
to support [any access control pattern your heart desires][patterns], but for
this example we'll focus on attribute-based access control (ABAC).
...
First, we're going to install the `oso` and `sqlalchemy-oso` packages:
...
Before we start filling that policy file with authorization rules, we need to
register the application data types that we're going to use in our authorization
policy with Oso.
...
In addition, let's register the `BearBase` Pydantic model that's used in the
`create()` handler:
...
Finally, let's enforce authorization in the `create()` handler so that those
...
will return a `403 Forbidden`. Oso is deny-by-default, and we currently have an
empty policy file. In the next section, we'll write our first authorization
...
And we are checking that the `is_banned` field is false on the `user`.
We don't yet need to check anything further about the `bear` resource,
so we prefix it with an underscore to tell Oso this input is not used.
...
Save the file, flip back to [localhost:8080][], and you should once again be
...
issue, we could load the collection from the database and iterate over it,
...
Without further ado, let's wire up our app so that we can efficiently filter
some bears.

First, we need to import `authorized_sessionmaker()` from the `sqlalchemy-oso`
library:
...
Next, we'll create a new [FastAPI dependable][] that mirrors our existing
...
To start off, let's add a rule to `app/policy.polar` that permits all users to
list all bears:
...
Next, let's add some constraints to our `"index"` rule for bears so we can see
...
SQLAlchemy. We created and configured a new Okta application to handle identity
management and authentication for our app. Then we used Oso to add efficient,
fine-grained authorization to our back end API.
