...
Edit your Okta app's SAML settings again and fill in the *Group Attribute Statements* section.
...
Then, create a `SecurityConfiguration` class that overrides the default configuration and uses a converter to converts the values in the `groups` attribute into Spring Security authorities.
...
If you configured your app to use these values, authentication would work, but you wouldn't be able to log out. Scroll to the top of page, select *Addons* and enable SAML.
...
One quick way to see this app working in a production environment is to deploy it to Heroku. https://devcenter.heroku.com/articles/heroku-cli[Install the Heroku CLI] and create an account to begin.
...
For authentication to work with SAML, you'll need to update your Okta and/or Auth0 apps to use your Heroku app's URL in place of `\http://localhost:8080`, wherever applicable.

== Learn more about Spring Boot and Spring Security

I hope you've enjoyed learning how to use Spring Security to add SAML authentication. Integration was as simple configuring a metadata URI and only became more complicated when you added a logout feature. The ability to convert groups from your identity provider to authorities is pretty slick too!

You can find the code for this example on GitHub, in the https://github.com/oktadev/okta-spring-boot-saml-example[@oktadev/okta-spring-boot-example] repository.
