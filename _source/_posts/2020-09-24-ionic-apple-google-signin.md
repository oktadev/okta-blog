...
<!--
Add Social Login to Ionic Apps is 57
Social Login with Ionic 51
Use Sign In with Apple/Google/* in Ionic gets a 58
Sign In to Ionic with *! gets a 60
Sign In to Ionic with All The Things! is a 69
Sign In to Ionic with Google and Apple is 61
Add Apple and Google Sign-In to Ionic Apps gets a 57
Ionic + Apple and Google Sign In 56
Add Social Login with Apple and Google to Ionic Apps 59
-->

Apple announced a Sign in with Apple service at their main developer conference in June 2019. If you're familiar with social login with Google or Facebook, it's very similar. Most of these identity services use OAuth + OpenID Connect (OIDC), and Apple's implementation is similar. 

Today I'd like to show you how to develop a mobile application with Ionic, add OIDC authentication, retrieve the user's information, and add social login (aka federated identity) with Apple and Google. I'll even show you how to test it works in a phone simulator, as well as on your device.
...
Then, use `ionic start` to create a new Angular app with tabs. You might notice this command also specifies Capacitor. This is the recommended way to run Ionic apps on mobile devices in 2020+.
...
If you're using the command line, you'll have to use your browser to adjust the redirect URIs. Your app's settings should look similar to the screenshot below when you're done.
...
This will prompt you for an issuer and client ID. If you used your browser, the client ID will be displayed on your screen. You can find the issuer in your Okta dashboard at **API** > **Authorization Servers**. It usually looks something like `https://dev-133337.okta.com/oauth2/default`.
...
You can see all the files that are added [in the OktaDev Schematics project](https://github.com/oktadeveloper/schematics/tree/main/src/add-auth/ionic/angular/src/app).
...
One of the neat things you can do with your Okta developer account is you can add additional identity providers. This means you can add Apple as an identity source and easily add Sign in with Apple to your Ionic app!
...
### Create an Services ID on Apple
...
Select the **Sign In with Apple** check box, and then click **Configure** to define the domain your app is running on and the redirect URLs used during the OAuth flow.
...
Click **Done**, and then on the **Keys** page, click the key that you just created and make note of the Key ID. You need it in the next section.

Make note of your Team ID in the upper-right corner of your Apple developer dashboard (just below your account name).
...
Okta provides an [Angular SDK](https://github.com/okta/okta-oidc-js/tree/master/packages/okta-angular) as well as a [Sign-In Widget](https://github.com/okta/okta-signin-widget). I've encountered quite a few developers that want to have an in-app login screen without popping a browser. 
...
To use a custom domain with Okta, you need to have access to its DNS settings and be able to create a TLS certificate. 
...
Since you've added a new domain, you've created another possible redirect URL. Log in to your [Apple developer account](https://developer.apple.com) and go the Service ID you created. Edit your service and configure Sign In with Apple. Add a new Website URL with your domain and authorize callback. 
...
In Okta, go to **Customization** > **Signin Page**. Make sure you're using the latest version, then modify the HTML on the right. Find the line with `var config` and add the following after it, where `<apple-idp-id>` can be found at **Users** > **Social & Identity Providers** > **(>)** before Apple
...
If you want to add Login with Google, you can do it with very similar steps. Rather than repeat all the steps verbatim here, I invite you to check out Okta's [Google Social Login Guide](https://developer.okta.com/docs/guides/add-an-external-idp/google/create-an-app-at-idp/). 

**TIP**: You'll need to add `http://localhost:8100` as a JavaScript origin in order to login with Google via your browser in development.
...
If you liked this post, follow [@oktadev](https://twitter.com/oktadev) on Twitter to see when we publish in-depth tutorials on Java, JavaScript, .NET, Python, PHP, and many others. My team also has [a YouTube channel](https://youtube.com/oktadev). You should [subscribe](https://youtube.com/c/oktadev?sub_confirmation=1)! 😊
