...
const CompanyTableContainer = (props: CompanyTableProperties) => {
  ...
  const { data: dataList, loading: loadingList, error: errorList } = useAsync(() => CompanyApi.getCompanyList({ page: page }), [page]);
  const { data: dataCount } = useAsync(() => CompanyApi.getCompanyCount(), []);
...
Then in the command line login to Auth0 with the CLI:
...
----
  ...
  const { data: dataList, loading: loadingList, error: errorList } = useAsyncWithToken(() => CompanyApi.getCompanyList({ page: page }), [page]);
