...
CAUTION: Add `*.env` to your `.gitignore` file, so you don't accidentally check in any secrets!
...
NOTE: Your JHipster Registry is now configured to use Okta for authentication. However, you'll need to add redirect URIs in your Okta app for `http://localhost:8761`.
...
You should be able to open `http://localhost:8080`, sign in, and access all of your microservices.
