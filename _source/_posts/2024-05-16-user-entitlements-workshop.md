...
When you work on B2B SaaS apps used by large customer organizations, synchronizing those customers' users within your software system can be tricky! You must synchronize user profile information and the user attributes needed for access management. For customers with large workforces, there may be thousands of users to manage, and those demanding customers require a speedy onboarding process, including automated user provisioning from their identity provider! 

Firstly, congratulations on the success of your application for winning large customer accounts—it must be due to the excellent software engineering! 🏆 With all those big sales, it's now time to figure out how to manage users and access levels at scale from an identity provider to your system in the [Enterprise-Ready and Enterprise-Maturity workshops](/blog/tags/enterprise-ready-workshops/) series.
...
Managing users across domains is critical to making B2B apps enterprise-scalable. In the on-demand workshop series, we tackle dilemmas faced by the developers of SaaS products wanting to scale their apps to enterprise customers by iterating on a fictitious B2B Todo app to make it more secure and capable for enterprise customers. In this workshop, you'll add the Todo app's automated user provisioning capabilities supporting user profiles and access management attributes. If you want to skip to the completed code project for this workshop, you can find it in the [`entitlements-completed` branch on the GitHub repo](https://github.com/oktadev/okta-enterprise-ready-workshops/tree/entitlements-workshop-complete).

**Table of Contents**{: .hide }
* Table of Contents
{% include toc.md %}

The Todo app tech stack uses a React frontend and Express API backend. For this workshop, you'll need the following required tooling:

**Required tools**
 * [Node.js](https://nodejs.org/en) v18 or higher
 * Command-line terminal application
 * A code editor/Integrated development environment (IDE), such as [Visual Studio Code](https://code.visualstudio.com/)
...
We're adding new API endpoints in this workshop, so you need an HTTP Client testing tool and the required tooling listed in the getting started guide.

## Manage users at scale using System for Cross-domain Identity Management (SCIM)

In this workshop, you'll add the user attributes required for access management, otherwise known as user entitlements. You'll build upon a prior workshop that introduced syncing users across systems using the [System for Cross-domain Identity Management](https://scim.cloud/) (SCIM) protocol. Then, we will continue diving deeper into supporting user entitlements using the SCIM protocol.

Before we get going with user entitlements, you'll first step through the interactive and fun [Enterprise-Ready Workshop: Manage Users with SCIM](/blog/2023/07/28/scim-workshop) workshop to get the SCIM overview, set up the code and your Okta account, and to see how the protocol works. I'll settle down with a cup of tea and a good book and wait while you learn about SCIM and are ready to continue! 🫖🍵📚

{% excerpt /blog/2023/07/28/scim-workshop %}

### Prepare the Express.js API project

Start from a clean code project by using the SCIM workshop's completed project code from the [scim-workshop-complete](https://github.com/oktadev/okta-enterprise-ready-workshops/tree/scim-workshop-complete) branch. I'll post the instructions below using [Git](https://git-scm.com/), but you can download the code as [a zip file](https://github.com/oktadev/okta-enterprise-ready-workshops/archive/refs/heads/scim-workshop-complete.zip) if you prefer and skip the Git command.
...
### Serve the Express API and test the SCIM endpoint
...
This command serves the API on port 3333. Launch your HTTP client and make a call to the `/Roles` endpoint by calling
...
The project uses [Morgan](https://github.com/expressjs/morgan), a library that automatically adds HTTP logging to the Express API. The terminal output includes `POST` and `PUT` request payloads, so it's an excellent way to keep track of the SCIM calls as you work through the workshop. 
...
First, we need to serve the API so Okta can access it. You'll use a temporary tunnel for local development that makes `localhost:3333` publicly accessible. I'll use an NPM library that we don't have to install or sign up for, as shown in the instructions below, but feel free to use your favorite tunneling system if you have one.

You need two terminal sessions.

In one terminal, serve the API using the
...
which creates a tunnel for the application serving on port 3333. The console output displays the tunnel URL in the format `https://{yourTunnelSubdomain}.loca.lt`, such as
...
## Create an Okta SCIM with Entitlements application

In the pre-requisite SCIM workshop, you added a SCIM application in Okta to connect to the Todo app. We must do something similar to connect SCIM with entitlements support. 

Sign into your [Okta Developer Edition account](https://developer.okta.com/login/). In the Admin console, navigate to **Applications** > **Application**. Press the **Browse App Catalog** button to create a new Okta SCIM application.

In the search bar, search for "(Header Auth) Governance with SCIM 2.0" and select the app.
...
Stop the tunnel and the API using the <kbd>Ctrl</kbd>+<kbd>c</kbd> command in the terminal windows. We'll make some changes to the API that won't automatically reflect in the local tunnel, so we'll get all our entitlements changes made and re-sync with Okta.

## SCIM schemas and resources 

In the first SCIM workshop, you learned about SCIM's `User` resource and built out operations around the user. You updated only a handful of user properties in the workshop, but SCIM is way more powerful thanks to its superpower—_extensibility_. ✨ There are more resource types defined in SCIM beyond User!
...
### Harness TypeScript to conform to schemas
...
The API response must match the list format. Open `okta-enterprise-ready-workshops/apps/api/src/entitlements.ts`. 

You need to import `IListResponse` and `SCHEMA_LIST_RESPONSE` to the imports from the `scim-types` file:
...
### Return roles from the database from the SCIM `/Roles` endpoint
...
SCIM list responses always wrap multi-value resources, like the list of roles or resource types.

The `IListResource` interface must support `IOktaRole` and `IResourceType`. We can support different list response objects using [generics](https://www.typescriptlang.org/docs/handbook/2/generics.html) and [union types](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#union-types). Update the `IListResource` to match the code below.
...
You'll see errors in the IDE and the console if you're running the API. No worries; we'll fix those errors soon!
...
Then create the `/ResourceType` route by adding the code below the `rolesRoute`
...
## Add roles to SCIM user requests
...
and change it and the code after that line to
...
const updatedUser = await prisma.user.update({
  data: {
    ...
    roles: {
      connect: roles?.map(role => ({id: parseInt(role.value)})) || []
    }
  },
  where : {
    id
...
so we can work with the user's updated roles and save the changes in the database.
...
What can Okta do with user entitlements? For defined entitlements, like what we have with roles, Okta can discover the roles you define for the Todo app and ingest existing roles on users. Now that you have all the endpoints needed for a SCIM client to discover resources held by a SCIM server, you can see this in action on Okta.

You'll need to serve the API and create a local tunnel. Serve the API using the `npm run serve-api` command. Run `npx localtunnel --port 3333` in a second terminal window. Take note of your tunnel URL.
...
Navigate to **Import** and press the **Import Now** button. Okta scans the users in the todo app, but since there are no new users, there's no confirmation process. The user scan synced the existing users and the roles! 
...
Let's say you have both user roles but want to add a custom entitlement, such as licenses, profiles, or something else. Let's walk through the example where we want to add a custom entitlement. We will call this "Characteristic," such as whether the user is cool. We know Trinity is cool, so it's just logical to note their coolness as part of their user attributes. Would an uncool person have a cool name like that?

SCIM clients must discover resources through schemas. So, we first need to define the schema describing "Characteristic". We can extend from an existing schema, such as Okta's entitlement schema, which tracks data as a key-value pair and add our own flavoring to it.
...
We defined a new schema URN for characteristic. Following naming conventions for extension schemas, we substituted our company name (Best Apps) and added the app's name (Todo app). The format looks like this
...
Characteristic is an unique resource type because it's a new, custom type extending from an existing schema. We must explicitly show this relationship for consuming SCIM clients, like Okta. Find the `IResourceType` interface. We'll add a new optional property, `schemaExtensions` and inline the type definition.
...
SCIM clients expect a list of schemas you offer in the SCIM server. You might've guessed what that means. You must wrap all the schemas in a SCIM `ListResponse`. Find `IListResponse` and add `ISchema` as a supported type. The `IListResponse` interface changes to:
...
With all the types and interfaces defined, it's time to move to writing the code for the route. Open `okta-enterprise-ready-workshops/apps/api/src/entitlements.ts`.
...
schemasRoute.route('/')
  .get((_, res) => {
    const characteristic: ISchema = {
      ...
      attributes: [{
        name: 'is_cool',
        description: 'Profile entitlement extension for coolness factor',
        type: 'string',
        multiValued: false,
        required: false,
...
And we must define a route for `/Characteristics`, in the same way one exists for `/Roles`. We won't worry about updating the database for this. We'll hardcode the characteristic for now so you can see what this looks like in Okta. Feel free to add the required code to connect it to the database as homework. 🏆 Add the following code below the schemas route:
  ...
  .get((_, res) => {
    const characteristicsListResponse: IListResponse<ICharacteristic> = {
      ...
      Resources: [{
        ...
        type: "Characteristic",
        id: "is_cool",
        displayName: "This user is so cool"
      }]
    };
...
Lastly, we must add the new characteristic resource type to the `/ResourceTypes` response so Okta knows the resource exists. Find the `resourceTypes.route('/')` definition and update the `resourceTypes` array to include both roles and characteristics.
...
Navigate to the **Governance**. It may take 10-15 minutes for **Characteristic** to populate here, but you'll see the display name and value when it does. Go **< Back to application** and navigate to **Assignments**. Open the user context menu for "Trinity" by pressing the three vertical dots icon menu and opening **View entitlements**. Press **Edit** and **Customize entitlements** to add the `is_cool` user characteristic. **Save** the changes and navigate back to the Okta SCIM app.
...
<tr>
    ...
    <td markdown="span">
      **Note** <br/>
      We used an API key for demonstration purposes, but we recommend using OAuth to secure the calls from Okta to your API for production applications. It's too much to include in this post, but be on the lookout for an upcoming workshop using OAuth.
    </td>
</tr>
</table>
...
In this workshop, you dived deeper into SCIM and learned about resources and schemas. You also synced users and their pre-existing entitlements from the Todo app and provisioned users within Okta. I hope you enjoyed this workshop and have ideas for using it for your SaaS applications! To read more about Okta Identity Governance, check out the [Identity Governance](https://help.okta.com/oie/en-us/content/topics/identity-governance/iga.htm) help docs.
