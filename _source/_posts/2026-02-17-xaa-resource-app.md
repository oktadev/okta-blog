---
layout: blog_post
title: "Test Your Resource App is Connected to Okta and XAA-enabled"
...
In this tutorial, we'll demonstrate how to test that an XAA-enabled resource app you have created (**TaskFlow**) is correctly using Okta as its **enterprise Identity Provider (IdP)** and source of truth. To do this, you'll:
...
> Note that your requesting app (Agent0) does not need Okta to act as its authorization server.
...
Finally, configure Agent0 with the location of your resource app
...
* Click Save.  
* Locate and copy the Client ID and Client secret in the Sign-On methods section. You must include these in the `.env.agent` file of your app.

> Note: If you are using Okta as your requesting app's Identity Provider, only the org auth server can be used to exchange ID-JAG tokens.
...
To set up your resource app in your org, you can use the placeholder integration in the OIN catalog called ***Todo0 - Cross App Access (XAA) Sample Resource App*** and point it at your resource app instead.
